...
def load_dify_workflow_bindings(doc_path: str = str(API_DOC_PATH)) -> Dict[str, DifyWorkflowBinding]:
    path = Path(doc_path)
    if not path.exists():
        example_path = API_DOC_EXAMPLE_PATH
        if example_path.exists():
            path = example_path
        else:
            raise FileNotFoundError(f"Dify API 文档不存在: {path}")
...
def dify_workflows_available() -> bool:
    ...
    required = {
        DOCUMENT_READ_DSL,
        FORMAT_REVIEW_DSL,
        REFERENCE_REVIEW_DSL,
        HALLUCINATION_REVIEW_DSL,
        REPORT_GENERATION_DSL,
        REPORT_QA_DSL,
    }
    if not required.issubset(set(bindings)):
        return False
    return all(
...
def run_dify_review_chain(
    ...
    *,
    ...
    paper_bundle, track, reasons = _build_paper_bundle(
        path,
        template_name=template_name,
    ...
    paper_profile = doc_output.get("paper_profile") or {}
    ...
    section_digest = doc_output.get("section_digest") or []
    evidence_index = doc_output.get("evidence_index") or []
    risk_hints = doc_output.get("risk_hints") or []
    ...
    format_review = _loose_json_loads(_extract_output(format_raw, ["format_review_json"]))

    reference_verify = paper_bundle.get("reference_stats") or {}
    ...
    reference_review = _to_mapping(_extract_output(reference_raw, ["reference_review_json"]))
    ...
    format_review["issues"] = _ensure_issue_ids(format_review.get("issues", []), "format", "format")
    reference_issues = _reference_items_to_issues(reference_review)
    reference_section = {
        **reference_review,
    ...
    evidence_records = _normalize_evidence_records(report_delta, format_review, reference_review, hallucination_review)
