...
from article_check.runtime import build_runtime, create_paper_task
        ...
        if config.deepseek.api_key:
            ...
            if self.input().lower() in ("y", "yes", "是", ""):
                ...
                try:
                    ...
                    task = create_paper_task(p)
                    result = asyncio.run(runtime.orchestrator.review_single(task))
    ...
    def handle_batch(self, params: dict):
        ...
        if self.input().lower() in ("y", "yes", "是"):
            ...
            try:
                ...
                tasks = [create_paper_task(p["path"]) for p in papers]
                results = asyncio.run(runtime.orchestrator.review_batch(tasks, max_concurrent=4))
                self.say("批量审查完成！")
                for r in results:
