...
class ArticleCheckState {
    constructor() {
        ...
        this.lastReportJsonPath = null;
        this.lastBatchPayload = null;
        this.lastBatchJsonPath = null;
        this.lastReviewedFilePath = null;
    }
}
class WorkflowTreeProvider {
    ...
    getChildren() {
        ...
        return Object.entries(graph).map(([nodeId, node]) => {
            ...
            item.iconPath = new vscode.ThemeIcon(node.status === "completed" ? "pass-filled" :
...
class EvidenceTreeProvider {
    ...
    getChildren() {
        ...
        return evidence.map((record) => {
            const item = new vscode.TreeItem(`${record.stage ?? "-"}: ${record.claim ?? "-"}`, vscode.TreeItemCollapsibleState.None);
            ...
            item.iconPath = new vscode.ThemeIcon(record.severity === "critical" ? "error" :
...
class ArticleCheckWorkbenchProvider {
    constructor(context, state, output, workflowTreeProvider, evidenceTreeProvider, diagnosticCollection) {
        this.context = context;
        this.state = state;
        this.output = output;
        this.workflowTreeProvider = workflowTreeProvider;
        this.evidenceTreeProvider = evidenceTreeProvider;
    ...
    resolveWebviewView(webviewView) {
        ...
        webviewView.webview.onDidReceiveMessage(async (message) => {
            if (message.type === "reviewCurrent") {
                await reviewCurrentFile(this.context, this.state, this.output, this, this.workflowTreeProvider, this.evidenceTreeProvider, this.diagnosticCollection);
            }
            if (message.type === "reviewWorkspace") {
                await reviewWorkspacePapers(this.context, this.state, this.output, this, this.workflowTreeProvider, this.evidenceTreeProvider, this.diagnosticCollection);
            }
...
ArticleCheckWorkbenchProvider.viewType = "articleCheck.workbench";
let lastPanel = null;
let lastDetailPanel = null;
function activate(context) {
    ...
    const provider = new ArticleCheckWorkbenchProvider(context, state, output, workflowTreeProvider, evidenceTreeProvider, diagnosticCollection);
    context.subscriptions.push(output, diagnosticCollection, vscode.window.registerWebviewViewProvider(ArticleCheckWorkbenchProvider.viewType, provider), vscode.window.registerTreeDataProvider("articleCheck.workflowTree", workflowTreeProvider), vscode.window.registerTreeDataProvider("articleCheck.evidenceTree", evidenceTreeProvider), vscode.commands.registerCommand("articleCheck.reviewCurrentFile", async () => {
        await reviewCurrentFile(context, state, output, provider, workflowTreeProvider, evidenceTreeProvider, diagnosticCollection);
    }), vscode.commands.registerCommand("articleCheck.reviewWorkspace", async () => {
        await reviewWorkspacePapers(context, state, output, provider, workflowTreeProvider, evidenceTreeProvider, diagnosticCollection);
    ...
    }), vscode.commands.registerCommand("articleCheck.openWorkflowNodeDetail", async (nodeId) => {
        await openWorkflowNodeDetail(context, state, nodeId);
    }), vscode.commands.registerCommand("articleCheck.openEvidenceDetail", async (evidenceId) => {
...
async function reviewCurrentFile(context, state, output, provider, workflowTreeProvider, evidenceTreeProvider, diagnosticCollection) {
    ...
    const payload = JSON.parse(fs.readFileSync(tempJsonPath, "utf8"));
...
async function reviewWorkspacePapers(context, state, output, provider, workflowTreeProvider, evidenceTreeProvider, diagnosticCollection) {
    ...
    state.lastBatchPayload = JSON.parse(fs.readFileSync(tempJsonPath, "utf8"));
    const firstItem = state.lastBatchPayload.items?.[0];
    if (firstItem) {
        state.lastReportPayload = firstItem;
        state.lastReportJsonPath = firstItem.summary?.report_json_path ?? null;
...
async function runSingleReview(filePath, output) {
    const ext = path.extname(filePath).toLowerCase();
    const config = vscode.workspace.getConfiguration("articleCheck");
    const pythonPath = config.get("pythonPath", "python");
    const reviewDepth = config.get("reviewDepth", "auto");
    const workspaceFolder = vscode.workspace.getWorkspaceFolder(vscode.Uri.file(filePath))?.uri.fsPath ??
        path.dirname(filePath);
    const tempJsonPath = path.join(os.tmpdir(), `article-check-${Date.now()}-${path.basename(filePath, ext)}.json`);
    output.clear();
...
async function askQuestionWithReport(reportJsonPath, question) {
    const config = vscode.workspace.getConfiguration("articleCheck");
    const pythonPath = config.get("pythonPath", "python");
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ??
    ...
    const answerPayload = JSON.parse(result.stdout || "{}");
    return answerPayload.answer ?? "未获得有效回答。";
}
async function openFormalReport(state) {
...
async function openWorkflowNodeDetail(context, state, nodeId) {
    const payload = state.lastReportPayload;
...
async function openEvidenceDetail(context, state, evidenceId) {
    const payload = state.lastReportPayload;
...
function openReportPanel(context, payload, reportJsonPath, initialFocusTargetId) {
    const panel = vscode.window.createWebviewPanel("articleCheckReport", `Article Check: ${payload.meta?.paper_title ?? "审查报告"}`, vscode.ViewColumn.Beside, {
        enableScripts: true,
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
        if (message.type === "askQuestion") {
            ...
            try {
                ...
                const config = vscode.workspace.getConfiguration("articleCheck");
                const pythonPath = config.get("pythonPath", "python");
                const workspaceFolder = (editor && vscode.workspace.getWorkspaceFolder(editor.document.uri)?.uri.fsPath) ??
...
function renderWorkbenchHtml(state) {
    const payload = state.lastReportPayload;
...
function renderReportHtml(payload, initialFocusTargetId = null) {
...
function focusReportFragment(context, state, targetType, targetId) {
    if (!state.lastReportPayload) {
...
function openDetailPanel(context, title, html, onMessage) {
    const panel = vscode.window.createWebviewPanel("articleCheckDetail", title, vscode.ViewColumn.Beside, {
        enableScripts: true,
        retainContextWhenHidden: true
    });
...
function applyDiagnostics(state, diagnosticCollection) {
    diagnosticCollection.clear();
    ...
    findings.forEach((finding, index) => {
        diagnostics.push(new vscode.Diagnostic(buildRangeFromLocation(finding.location), buildDiagnosticMessage(finding.description ?? "发现问题", finding.suggestion ?? null, finding.category ?? null), mapSeverity(finding.severity)));
    ...
    evidence.forEach((record, index) => {
        diagnostics.push(new vscode.Diagnostic(buildRangeFromLocation(record.location), buildDiagnosticMessage(record.claim ?? "发现证据项", record.suggestion ?? null, record.stage ?? null), mapSeverity(record.severity)));
        diagnostics[diagnostics.length - 1].source = "Article Check";
...
function parseLineNumber(location) {
    if (!location) {
        return null;
    }
...
function buildDiagnosticMessage(description, suggestion, category) {
...
function buildReportTargetId(targetType, targetId) {
